#  Changed prompt generation system
import os
import re
import json
import itertools
...
def generate_files(w2d, str_in, vars, txt_path, json_path, batch_size=1):
    txt_batch = []
    json_batch = []
    for i in range(21):
        combination = []
...
def write2files():
    with open('empty_tags/scatter_empty_tags_prev.txt', 'r') as txt_in:
        lines = txt_in.readlines()

    num_workers = cpu_count()
    pool = Pool(num_workers)
    txt_path = "prompts/scatterV5.txt"
    json_path = "json/scatterV5.json"
